...
pub unsafe fn MAU_SQRT_TIMEOUT() -> c_ulong {
    HCLK_VALUE() / (10000 as c_ulong)
}
...
pub const fn SMC_ADDR_SHIFT(
    mem_base_addr: uint32_t,
    mem_width: uint32_t,
    addr: uint32_t,
) -> uint32_t {
    if EXMC_SMC_MEMORY_WIDTH_16BIT == mem_width {
        mem_base_addr + (addr << 1)
    } else {
        mem_base_addr + (addr << 2)
    }
}
